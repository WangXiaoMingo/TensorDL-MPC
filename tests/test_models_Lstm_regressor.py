...
if __name__ == '__main__':
    ...
    from src.dlmpc import SeriesLstm, NormLstm, LSTM, ResnetLstm, SkipLstm, ResSkipLstm
    ...
    # x_sequences, u_sequences, y_sequences = window_generator.generate_2DT_sequences()
    ...
    models = {}
    # models['SeriesLstm'] = SeriesLstm(lstm_blocks=1).build(lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='1D')
    # models['NormLstm'] = NormLstm(lstm_blocks=1).build(lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='1D')
    models['LSTM'] = LSTM().build(lstm_blocks=1, lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='2DT')
    # models['ResnetLstm'] = ResnetLstm(lstm_blocks=1).build(lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='2DT')
    # models['SkipLstm'] = SkipLstm(lstm_blocks=1).build(lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='2DT')
    # models['ResSkipLstm'] = ResSkipLstm(lstm_blocks=1).build(lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='2DT')

    for model_name, model in models.items():
        '''data_type can select: 2DT, 2DT, 2DTT'''
        # train model and load best model
        if train:
            TrainModel(model,lr = 0.005,epoch=200).train_model(train_data,valid_data,show_loss=True)
        ...
        # 将DataFrame转换为表格
        tab = ax.table(cellText=test_result.values, colLabels=test_result.columns, rowLabels=test_result.index,
                       loc='center')
        # 可以为表格添加样式
