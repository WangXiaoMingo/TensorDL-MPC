...
def generate_sequences(data_y,data_u,delay_u,delay_y):
    start_seq = max(input_window_dy,input_window_dy,delay_u)
    num_samples = len(data_y)-start_seq
    state_num = input_window_dy + input_window_du
    x = np.zeros((num_samples, state_num))  # 系统状态
    y = np.zeros((num_samples, 1))
    for i in range(start_seq, len(data_y)):
        # 生成状态数据窗口,self.x由两部分构成
        temp_y_state = data_y[i - delay_y : i - delay_y + input_window_dy]
        temp_u_state = data_u[i - delay_u : i - delay_u + input_window_du]
...
x,y = generate_sequences(data_y,data_u,delay_u,delay_y)
...
'''===============3.导入模型并训练=============='''
pipelines = {}
pipelines['LR1'] = Pipeline([('LR', LinearRegression())])
pipelines['LR2'] = Pipeline([('max', MinMaxScaler()), ('LR', LinearRegression())])
pipelines['LR3'] = Pipeline([('scar', StandardScaler()), ('LR', LinearRegression())])
# pipelines['EN'] = Pipeline([('scar', StandardScaler()), ('EN', ElasticNet())])
pipelines['Lasso'] = Pipeline([('scar', StandardScaler()), ('Lasso', Lasso(alpha=0.00010))])
