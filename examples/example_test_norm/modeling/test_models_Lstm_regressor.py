...
if __name__ == '__main__':
    ...
    # model = SeriesLstm(lstm_blocks=1).build(lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='1D')
    model = NormLstm(lstm_blocks=1).build(lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='1D')
    # model = LSTM().build(lstm_blocks=1, lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='2DT')
    # model = ResnetLstm(lstm_blocks=1).build(lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='2DT')
    # model = SkipLstm(lstm_blocks=1).build(lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='2DT')
    # model = ResSkipLstm(lstm_blocks=1).build(lstm_units=32, dense_units=64, dim_u=1, dim_x = dim_x, data_type='2DT')
    ...
    if train:
        if norm1:
            TrainModel(model,lr = 0.0005,epoch=200).train_model(train_data,valid_data,show_loss=True) # 全部数据
        else:
    ...
    data.columns = ['Linear', 'test_real']
    # data.to_excel('../model_result/Linear.xlsx')
