...
if __name__ == '__main__':
    ...
    from src.dlmpc import LinearRegression
    ...
    model = LinearRegression().build(dim_u=1, dim_x=dim_x, data_type='1D')
    ...
    if train:
        if norm1:
            TrainModel(model,lr = 0.01,epoch=200).train_model(train_data,valid_data,show_loss=True) # 全部数据
        else:
            TrainModel(model, lr=0.001, epoch=200).train_model(train_data, valid_data, show_loss=True)
