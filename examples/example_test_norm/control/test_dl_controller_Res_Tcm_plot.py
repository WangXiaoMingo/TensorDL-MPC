...
if __name__ == '__main__':
    ...
    u_bounds = [0,25]          # control variable constraints (u)
    ...
    '''
    state_y = tf.constant([[581.40], [583.06]], dtype=tf.float32)
    state_u = tf.constant([[5.562]], dtype=tf.float32)
    u0 = tf.constant([5.618], dtype=tf.float32)
    #
    # state_y = tf.constant([[609.844], [613.561]], dtype=tf.float32)  # 1800行左右
    # state_u = tf.constant([[0]], dtype=tf.float32)
    # u0 = tf.constant([0.608], dtype=tf.float32)
    ...
    if Train_NN:
        '''get trained model'''
        import os
        script_path = '../modeling/test_models_Resnet_Tcm_regressor.py'
        os.system(f'python {script_path}')

    model = load_model(f'../models_save/ResnetTcm_predictor.h5')
    # model.summary()
    ...
    if mpc:
        ...
        for i in range(N):
            ...
            parameter = mpc.make_step(error, y_ref, iterations=200, tol=1e-6)  #
            u0 = parameter['u0']
            # estimate state
            state_y, state_u, error,plant_output = mpc.estimate_simu(parameter['u0'])
        ...
        result.to_excel('../model_result/TCM_MPC.xlsx')
