...
if __name__ == '__main__':
    ...
    from src.dlmpc import SimSISO1
    ...
    Train_NN = False
    mpc = True
    predict_horizon = 3 #10  # 预测时域  (2,1), (4,2 *),(3,3)
    ...
    y_ref = 10  # 参考轨迹值
    # 初始化系统状态
    state_y = tf.constant([[1], [1.2],[0.8]], dtype=tf.float32)
    state_u = tf.constant([[0],[0.1]], dtype=tf.float32)
    u0 = tf.constant([0.2], dtype=tf.float32)

    # TODO: Step2: load plant and parameters
    '''return plant: simulation'''
    simulation = SimSISO1(noise_amplitude=0)

    # TODO: Step3: Load  NN model and training
    if Train_NN:
        '''get trained model'''
        import os
        script_path = 'test_models_linear_regressor.py'
    ...
    if mpc:
        mpc_controller = MPCController(model, predict_horizon, control_horizon, Q, R, ly, lu, dim_u, du_bounds,u_bounds, opt)
        # 创建数据存储和图表
        data = np.zeros((N - 2, 7))
